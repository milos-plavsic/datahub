...
- **(GMS / schemaField MCP side effects + system update)** Optional nested flags under `metadataChangeProposal.sideEffects.schemaField` can mirror dataset `domains` and `ownership` onto schemaField entities (`MCP_SIDE_EFFECTS_SCHEMA_FIELD_DOMAIN_ENABLED` / `MCP_SIDE_EFFECTS_SCHEMA_FIELD_OWNERSHIP_ENABLED`, both requiring `MCP_SIDE_EFFECTS_SCHEMA_FIELD_ENABLED`). Defaults remain off. Enabling increases write fan-out (one MCP per column per mirrored aspect change) and can change DOMAIN / OWNER policy matches for schemaField URNs once those aspects are present.
  ...
  - **Backfill / cleanup:** The non-blocking SystemUpdate step `GenerateSchemaFieldsFromSchemaMetadata` (`SYSTEM_UPDATE_SCHEMA_FIELDS_FROM_SCHEMA_METADATA_ENABLED`) re-ingests dataset `schemaMetadata`/`status` to trigger schemaField side effects. Its upgrade id is fingerprinted by the effective domain/ownership mirror flags (`schema-field-from-schema-metadata-v2-d{0|1}-o{0|1}`), so a **first-time** change to a new fingerprint schedules a fresh run on the next SystemUpdateNonBlocking: enable → backfill mirrored aspects; disable → delete field `domains`/`ownership` for the off flag(s). **That cleanup deletes the aspect on every field of each scanned dataset — including manually written field aspects, not only mirrored copies** (see **Known Issues**). Keep the MCP flags consistent on GMS, MCE, and the upgrade job.
