...
### Install the Plugin
```shell
pip install 'acryl-datahub[cassandra]'
```
...
| <div className="path-line"><span className="path-main">password</span></div> <div className="type-name-line"><span className="type-name">One of string(password), null</span></div> | Password credential associated with the specified username. <div className="default-line default-line-with-docs">Default: <span className="default-value">None</span></div> |
...
| <div className="path-line"><span className="path-prefix">cloud_config.</span><span className="path-main">token</span>&nbsp;<abbr title="Required if cloud_config is set">❓</abbr></div> <div className="type-name-line"><span className="type-name">string(password)</span></div> | The Astra DB application token used for authentication.  |
